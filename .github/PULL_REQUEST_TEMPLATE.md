...
- [ ] New/changed sizing tier (`.github/tiers.json`)
- [ ] `.github/environments.json` updated (required when adding/removing an environment or region)
...
- [ ] If a new region was added: `region_index` in `environments.json` is unique repo-wide (never reused)
